...
fn main() {
    ...
    let mut pt_convert = Command::new("json2csv")
        .arg("-F")
        .arg("--output")
        .arg("encounterlist.csv")
        .arg("EncounterList.json")
        .spawn()
        .expect("failed to convert to parquet");
    println!();

    let mut enc_convert = Command::new("json2csv")
        .arg("-F")
        .arg("--output")
        .arg("patientlist.csv")
        .arg("PatientList.json")
        .spawn()
        .expect("failed to convert to parquet");
    println!();
